...
async def standardization(input: TextInput):
    try:
        ...
        print(f"MAIN NER-Results: {ner_results}")
        # 初始化标准化服务
        standardization_service = StdService(
            model=input.embeddingOptions.model,
            db_path=f"db/{input.embeddingOptions.dbName}_faiss.db",
            idmap_path=f"db/{input.embeddingOptions.dbName}_faiss.db.idmap.txt"
        )

        # 获取识别到的实体
        entities = ner_results.get('entities', [])
        if not entities:
            return {"message": "No Finance terms have been recognized", "standardized_terms": []}

        # 标准化每个实体
        standardized_results = []
        for entity in entities:
            print(f"MAIN Entity: {entity}")
        ...
        return {
            "message": f"原文为:{ner_results.get('text', '')}, {len(entities)} Finance terms have been recognized and standardized",
...
async def ner(input: TextInput):
    try:
        ...
        print(f"MAIN NER NER-Results: {results}")
