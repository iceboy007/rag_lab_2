...
class NERService:
    ...
    gouravsinha/finance-NER
    """
    def __init__(self):
        # 初始化 NER 模型，使用 GPU 如果可用
        # self.pipe = pipeline("token-classification", 
        #                    model="gouravsinha/finance-NER",
        #                    aggregation_strategy='simple',
        #                    device=0 if torch.cuda.is_available() else -1)

        self.pipe = pipeline(Tasks.named_entity_recognition, '../iic/nlp_raner_named-entity-recognition_chinese-base-finance')
    ...
    def process(self, text, options, term_types):
        ...
        print(f"Input-text: {text}, NER-Result: {result}")
        return {
            "text": text,
